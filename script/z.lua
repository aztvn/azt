...
local aa = {
    function()
        ...
        z["AddMultiParagraph"] = function(self, items, gap)
            if not items or type(items) ~= "table" then return end
            local count = #items
            if count == 0 then return end
            local grp = self:AddGroup({ Columns = count, Gap = gap or 6 })
            local cols = {}
            for i = 1, count do
                local col = grp:AddElement()
                col:AddParagraph(items[i])
                cols[i] = col
            end
            return grp
        end

        z["AddSettingsPage"] = function(C, D, E)
            local idx = (type(D) == "string") and D or nil
            local f = (idx and E) or (type(D) == "table" and D) or {}
            local parent = C.Container
            if not parent then return end
            ...
            local titleLbl = u("TextLabel",{
                RichText=true,
                FontFace=Font.new("rbxasset://fonts/families/GothamSSm.json",Enum.FontWeight.SemiBold),
                Text=tostring(f.Title or "Settings"),
                TextSize=14,
                TextXAlignment=Enum.TextXAlignment.Left,
                BackgroundTransparency=1,
            ...
            local secObj = {
                Container = innerWrap,
                Type = "SettingsPage",
                ScrollFrame = C.ScrollFrame,
                _elementCount = 0,
            }
            local proxy = setmetatable({}, {
                __index = function(t, key)
                    if key == "AddElement" then return nil end
                    local origMethod = z[key]
                    if origMethod and type(origMethod) == "function" and key:match("^Add") then
                        return function(self, ...)
                            local el = origMethod(secObj, ...)
                            if el and el.Frame then
                                el.Frame.Size = UDim2.new(0.5, -3, 0, el.Frame.Size.Y.Offset)
                                pcall(function()
                                    if el.DescLabel then el.DescLabel.Visible = false end
                                    if el.LabelHolder then
                                        el.LabelHolder.Size = UDim2.new(1, -20, 1, 0)
                                    end
                                end)
                            end
                            return el
                    ...
                    return secObj[key] or z[key]
                end
            })
            return _addElementToSection(C, {Frame=wrap, Proxy=proxy, Type="SettingsPage"}).Proxy
        end
        ...
        z.MultiParagraph = z["AddMultiParagraph"]
        z.SettingsPage = z["AddSettingsPage"]
